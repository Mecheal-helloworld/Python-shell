    ...
    for weapon in weapons:
        ...
        for name in names[1:-1]:
            ...
            for one_name in one_names:
                ...
                if (one_name not in find_weapon_dicts) and (one_name != "") and (len(one_name) <= 10):
                    find_weapon_dicts.append((one_name, 999))
...
def get_find_triggers():
    ...
    for trigger in triggers:
        ...
        if my_trigger == "":
            continue
        if trigger not in find_trigger_dicts:
...
def get_find_weapons(using_weapons=True):
    ...
    for weapon in weapons:
        ...
        if my_weapon == "":
            continue
        if my_weapon not in find_weapon_dicts:
            find_weapon_dicts.append((my_weapon, freq+500))
...
def get_find_countrys():
    ...
    for country in countrys:
        ...
        if my_country == "":
            continue
        if my_country not in find_country_dicts:
...
def get_find_times():
    ...
    for time in times:
        ...
        if my_time == "":
            continue
        if my_time not in find_time_dicts:
...
def get_find_locations():
    ...
    for location in locations:
        ...
        if my_location == "":
            continue
        if my_location not in find_location_dicts:
...
def change(fileName):
    ...
    for example in examples:
        ...
        words_in_sentence = jieba.lcut(sentence,cut_all=True,HMM=True)
        words_in_sentence = sorted(words_in_sentence, key = lambda x:len(x), reverse=True)
        words_flag = []
        ...
        my_examples.append({"events": my_events, "arguments": my_arguments})
        f1.write("识别的触发词：\n")
