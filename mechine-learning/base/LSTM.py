...
def parser(x):
	...
	return datetime.datetime(int('20'+year), int(month), int(day))
...
# 加载数据
series = read_csv('LBMA-GOLD.csv', header=0, parse_dates=[0], index_col=0, squeeze=True, date_parser=parser)
...
for i in range(len(test_scaled)):
    ...
    # 对比真实值和预测值
	print('id=%d, Predicted=%f, Expected=%f' % (i+1, yhat, expected))
	csv_write.append([len(train) + i + 1,expected,yhat])
f = open('最后100条数据.csv', 'w', newline='')
writer  = csv.writer(f)
